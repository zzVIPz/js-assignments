...
function getRectangleString(width, height) {
  ...
  if (width < 2 || height < 2) throw new Error("Can't draw a rectangle");
  if (width === 2 && height === 2) {
    return cornerTop + cornerBottom;
  } else {
    width === 2
      ? (str = cornerTop)
      : (str =
          cornerTop.slice(0, 1) +
          widthLine.repeat(width - 2) +
          cornerTop.slice(1));
    if (height > 2)
      for (let i = 0; i < height - 2; i++) {
        str +=
          heightLine.slice(0, 1) + " ".repeat(width - 2) + heightLine.slice(1);
      }
    width === 2
      ? (str += cornerBottom)
      : (str +=
          cornerBottom.slice(0, 1) +
          widthLine.repeat(width - 2) +
          cornerBottom.slice(1));
  }
  return str;
}
...
function encodeToRot13(str) {
  return str.replace(/[a-zA-Z]/g, el => {
    let start = el <= "Z" ? 65 : 97;
    return String.fromCharCode(start + ((el.charCodeAt() - start + 13) % 26));
  });
}
