...
function getPolynom() {
  ...
  return function(x) {
    ...
    if (arr.length === 2) return arr[0] * x + arr[1];
    if (arr.length === 3) return arr[0] * x ** arr[0] + arr[1] * x + arr[2];
  };
}
...
function logger(func, logFunc) {
  ...
  return function(x) {
    let str = "";
    str += `${func.name}(${x}) starts\n` + `${func.name}(${x}) ends`;
    console.log(logFunc(str));
    return func(x);
  };
}
...
function partialUsingArguments(fn) {
  ...
  return function(x) {
    let str = "";
    let arrNew = [...arguments];
    arr.forEach(el => (str += el));
    arrNew.forEach(el => (str += el));
    return str;
  };
}
